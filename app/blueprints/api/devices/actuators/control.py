...
def control_actuator_by_type():
    ...
    try:
        ...
        target_actuators = [
            ...
            if str(a.get('actuator_type', '')).lower() == actuator_type.lower()
        ]
        ...
        return _success({
            "message": f"Processed {len(results)} actuators",
            "results": results
        })
...
def export_actuator_state_history_csv(actuator_id: int):
    try:
        ...
        return Response(csv_data, mimetype='text/csv', headers={
            'Content-Disposition': f'attachment; filename=actuator_{actuator_id}_state_history.csv'
        })
    except Exception as e:
        return _fail(str(e), 500)
...
def export_unit_actuator_state_history_csv(unit_id: int):
    try:
        ...
        csv_data = _to_csv(rows, headers)
        return Response(csv_data, mimetype='text/csv', headers={
            'Content-Disposition': f'attachment; filename=unit_{unit_id}_actuator_state_history.csv'
        })
    except Exception as e:
        return _fail(str(e), 500)
