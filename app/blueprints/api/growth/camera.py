...
@safe_route("Failed to start camera")
def start_camera(unit_id: int) -> Response:
    ...
    try:
        ...
        return _fail(f"Camera start failed: {exc!s}", 400)


@growth_api.post("/units/<int:unit_id>/camera/stop")
@safe_route("Failed to stop camera")
def stop_camera(unit_id: int) -> Response:
...
@safe_route("Failed to capture photo")
def capture_photo(unit_id: int) -> Response:
...
@safe_route("Failed to get camera status")
def get_camera_status(unit_id: int) -> Response:
...
def update_camera_settings(unit_id: int) -> Response:
...
@safe_route("Failed to stream camera feed")
def camera_feed(unit_id: int) -> Response:
