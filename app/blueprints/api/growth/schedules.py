...
def bulk_update_schedules(unit_id: int):
    ...
    try:
        ...
        for schedule_id in schedule_ids:
            ...
            if existing.unit_id != unit_id:
                results["failed"].append({
                    "id": schedule_id,
                    "error": "Schedule does not belong to this unit"
                })
                continue
            #TODO: can be added reason to update, the service can log it but i need to implent frontend part
            try:
                ...
                if success:
                    ...
                    results["failed"].append({
                        "id": schedule_id,
                        "error": f"Failed to {action} schedule"
                    })
            except Exception as e:
                results["failed"].append({
                    "id": schedule_id,
        ...
        logger.info(f"Bulk {action}: {len(results['success'])} success, "
...
def auto_generate_schedules(unit_id: int):
    ...
    try:
        ...
        if hours is None or intensity is None:
            return _fail(
                f"Lighting data for plant '{plant_type}' at stage '{current_stage}' "
                "is missing hours or intensity.",
                400,
            )
        ...
        plant_info = {
            "automation": {
                "lighting_schedule": {
                    stage_key: {"hours": hours, "intensity": intensity}
                },
