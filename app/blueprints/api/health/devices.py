...
def register_device_routes(health_api: Blueprint):
    ...
    @safe_route("Failed to get devices health")
    def get_devices_health() -> Response:
        """
        ...
        for unit_id, _runtime in runtimes.items():
            try:
    ...
    @safe_route("Failed to get sensor health")
    def get_sensor_health(sensor_id: int) -> Response:
        """
        ...
        if result.get("success"):
            ...
            logger.warning("Sensor health check failed for sensor %s: %s (type: %s)", sensor_id, error_msg, error_type)
    ...
    @safe_route("Failed to get actuator health")
    def get_actuator_health(actuator_id: int) -> Response:
        """
        ...
        return _success({"actuator_id": actuator_id, "health_history": history, "count": len(history)})

    @health_api.post("/actuators/<int:actuator_id>")
    @safe_route("Failed to save actuator health snapshot")
    def save_actuator_health(actuator_id: int) -> Response:
        """
        ...
        if history_id:
            return _success({"history_id": history_id, "message": "Health snapshot saved successfully"}, 201)
        else:
