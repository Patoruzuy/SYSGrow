...
models_bp = Blueprint("ml_models", __name__, url_prefix="/api/ml/models")
...
def promote_model_version(model_name: str):
    """
...
def get_models_status():
    ...
    try:
        container = _container()
        registry = container.model_registry if hasattr(container, 'model_registry') else None
...
def get_drift_metrics(model_name: str):
    """
    ...
    try:
        ...
        for model_name in model_names:
            try:
                ...
                logger.warning(f"Error getting metadata for {model_name}: {model_error}")
                comparison.append({
                    "name": model_name,
...
def retrain_model(model_name: str):
    ...
    try:
        ...
        days = training_config.get("days", 90)
...
def activate_model_version(model_name: str):
