...
def _resolve_plant_id(active_plant):
    ...
    if hasattr(active_plant, "plant_id"):
        return active_plant.plant_id
    if hasattr(active_plant, "id"):
        return active_plant.id
...
def _resolve_current_threshold(container, unit_id: int, user_id: int, active_plant=None) -> float:
    ...
    if active_plant is not None:
        ...
        if hasattr(active_plant, "soil_moisture_threshold_override"):
            override = active_plant.soil_moisture_threshold_override
        ...
        if override is not None:
            ...
            if hasattr(active_plant, "plant_type"):
                name = active_plant.plant_type or getattr(active_plant, "plant_name", None)
            elif isinstance(active_plant, dict):
...
def _calculate_statistical_predictions(current, simulated):
    ...
    current_health = calc_health(current["temperature"], current["humidity"], current["light_hours"], current["co2"])
    predicted_health = calc_health(
        simulated["temperature"], simulated["humidity"], simulated["light_hours"], simulated["co2"]
    )
    ...
    current_cost = calc_energy_cost(current["temperature"], current["light_hours"], current["co2"])
    predicted_cost = calc_energy_cost(simulated["temperature"], simulated["light_hours"], simulated["co2"])
    ...
    current_growth = calc_growth_rate(
        current["temperature"], current["humidity"], current["light_hours"], current["co2"]
    )
...
def _generate_what_if_recommendations(current, simulated, predictions):
    ...
    if abs(temp_diff) > 3:
        ...
        recommendations.append({"message": "Temperature change looks optimal for most plants.", "priority": "low"})
    ...
    if "vpd" in predictions:
        ...
        if vpd_status == "optimal":
            recommendations.append(
                {"message": "VPD will be in optimal range (0.8-1.2 kPa) - excellent for growth.", "priority": "low"}
            )
    ...
    if simulated["light_hours"] > 18:
        ...
        recommendations.append({"message": "Short light duration (<12h) may reduce growth rate.", "priority": "medium"})
    ...
    if simulated["co2"] > 1500:
        recommendations.append(
            {"message": "Very high CO₂ levels (>1500 ppm) - ensure adequate ventilation.", "priority": "high"}
        )
