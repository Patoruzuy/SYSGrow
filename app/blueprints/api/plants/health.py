...
def get_plant_health_history(plant_id: int):
    ...
    """
    logger.info(f"Getting health history for plant {plant_id}")
    try:
...
def get_plant_health_recommendations(plant_id: int):
    """Get health recommendations for a plant"""
    logger.info(f"Getting health recommendations for plant {plant_id}")
    try:
        container = get_container()
