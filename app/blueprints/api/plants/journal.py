...
@safe_route("Failed to fetch plants health data")
def get_all_plants_health() -> Response:
    """
        ...
        for plant in unit_plants:
            ...
            try:
                ...
                if latest_entries and len(latest_entries) > 0:
                    ...
                    logger.info("Plant %s health_status: '%s'", plant["plant_id"], health_status)
                    plant["current_health_status"] = health_status
                else:
                    plant["current_health_status"] = ""
            except Exception as e:
                logger.error("Could not get health status for plant %s: %s", plant["plant_id"], e, exc_info=True)
                plant["current_health_status"] = ""
...
@safe_route("Failed to fetch plants guide")
def get_plants_guide() -> Response:
    """
    ...
    try:
        ...
        logger.warning("Failed to load plants_info.json: %s", e)
...
@safe_route("Failed to fetch full plants guide")
def get_plants_guide_full() -> Response:
    """
    ...
    try:
        ...
        logger.warning("Failed to load plants_info.json for full guide: %s", e)
...
@safe_route("Failed to fetch plant detail")
def get_plant_detail(plant_id: int) -> Response:
    """
    ...
    try:
        ...
        logger.warning("Failed to load plants_info.json for plant detail: %s", e)
...
@safe_route("Failed to calculate disease risk")
def get_disease_risk() -> Response:
    """
    ...
    for unit in units:
        ...
        try:
            ...
            logger.warning("Failed to get sensor readings for unit %s: %s", unit_id, e)
...
@safe_route("Failed to fetch harvests")
def get_harvests() -> Response:
    """
...
@safe_route("Failed to fetch journal entries")
...
@safe_route("Failed to record observation")
...
@safe_route("Failed to record nutrients")
def create_nutrient_record() -> Response:
    """
...
@safe_route("Failed to record watering event")
def create_watering_record() -> Response:
    """
