...
def update_plant_stage(plant_id: int) -> Response:
    ...
    logger.info("Updating stage for plant %s", plant_id)
...
@safe_route("Failed to set active plant")
def set_active_plant(unit_id: int, plant_id: int) -> Response:
    """Set a plant as the active plant for climate control"""
    logger.info("Setting plant %s as active in growth unit %s", plant_id, unit_id)
    # Verify unit exists
