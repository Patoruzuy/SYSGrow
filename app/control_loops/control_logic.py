...
class ControlLogic:
    ...
    def register_actuator(self, actuator_type: ActuatorType, actuator_id: int) -> None:
        ...
        logger.info("Registered actuator: %s -> ID %s", actuator_type.value, actuator_id)
    ...
    def unregister_actuator(self, actuator_type: ActuatorType) -> None:
        ...
        if actuator_type in self.actuator_map:
            ...
            logger.info("Unregistered actuator: %s", actuator_type.value)
    ...
    def _execute_actuator_command(self, actuator_id: int, command: str, strategy: ControlStrategy) -> bool:
        ...
        try:
            ...
            logger.error("Exception executing actuator command: %s", e)
            metrics.failed_actions += 1
            metrics.consecutive_errors += 1
            return False
    ...
    def _execute_actuator_level(self, actuator_id: int, level: float, strategy: ControlStrategy) -> bool:
        ...
        try:
            ...
            logger.error("Exception setting actuator level: %s", e)
            metrics.failed_actions += 1
            metrics.consecutive_errors += 1
            return False
    ...
    def control_temperature(self, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("Temperature control error: %s", e)
            return False
    ...
    def control_humidity(self, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("Humidity control error: %s", e)
            return False
    ...
    def control_co2(self, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("CO2 control error: %s", e)
            return False
    ...
    def control_lux(self, data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error("Lux control error: %s", e)
            return False
    ...
    def update_thresholds(self, data: dict[str, Any]) -> None:
        ...
        if "temperature" in data:
            ...
            logger.info("Updated temperature setpoint: %s°C", self.config.temp_setpoint)
        ...
        if "co2" in data:
            ...
            logger.info("Updated CO2 setpoint: %s PPM", self.config.co2_setpoint)

        if "lux" in data:
            self.lux_controller.setpoint = float(data["lux"])
            self.config.lux_setpoint = float(data["lux"])
            logger.info("Updated Lux setpoint: %s LUX", self.config.lux_setpoint)
    ...
    def update_pid_parameters(self, controller_name: str, kp: float, ki: float, kd: float) -> None:
        ...
        logger.info("Updated PID parameters for %s: Kp=%s, Ki=%s, Kd=%s", controller_name, kp, ki, kd)
