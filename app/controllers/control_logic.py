...
from app.domain.actuators import ActuatorType, ActuatorState
...
class ControlLogic:
    ...
    def __init__(
        ...
        self.temp_controller = PIDController(
            kp=self.config.temp_kp,
            ki=self.config.temp_ki,
            kd=self.config.temp_kd,
        ...
        self.co2_controller = PIDController(
            kp=self.config.co2_kp,
            ki=self.config.co2_ki,
            kd=self.config.co2_kd,
            setpoint=self.config.co2_setpoint
        )
        self.lux_controller = PIDController(
            kp=self.config.lux_kp,
            ki=self.config.lux_ki,
            kd=self.config.lux_kd,
            setpoint=self.config.lux_setpoint
        )
    ...
    def _get_actuator_id(self, actuator_type: ActuatorType) -> Optional[int]:
    ...
    def _execute_actuator_command(
        ...
        try:
            ...
            if metrics.consecutive_errors >= self.config.max_consecutive_errors:
                logger.critical(
                    f"Control strategy {strategy.value} disabled due to "
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'registered_actuators': {
                actuator_type.value: actuator_id
                for actuator_type, actuator_id in self.actuator_map.items()
            },
