...
class Zigbee2MQTTAdapter(ISensorAdapter):
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        """
        logger.warning(
            f"Zigbee2MQTTAdapter._on_mqtt_message() called directly - "
            f"this should not happen. MQTT subscriptions should be handled by MQTTSensorService."
        )
        try:
    ...
    def set_calibration_offset(self, sensor_type: str, offset: float) -> None:
        ...
        try:
            ...
            logger.info(
                f"Set {calibration_key} = {offset} for {self.friendly_name}"
            )
        except Exception as e:
