...
from app.domain.sensors import SensorEntity, SensorType, SensorConfig, Protocol
...
class SensorFactory:
    ...
    def create_sensor(
        ...
        logger.debug(f"Creating adapter for {sensor_type_str} with protocol {protocol.value}, params: {list(final_adapter_params.keys())}")
        ...
        logger.info(
            f"Created sensor: {name} (ID: {sensor_id}, "
            f"Type: {sensor_type_str}, Protocol: {protocol.value})"
        )
    ...
    def _create_processors(
        ...
        for processor_class in processor_classes:
            try:
                ...
                logger.error(
                    f"Failed to create processor {processor_class.__name__}: {e}"
                )
...
_global_factory: Optional[SensorFactory] = None
