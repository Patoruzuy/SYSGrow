...
class ContinuousMonitoringService:
    ...
    def __init__(
        ...
        self._on_critical_alert: Callable[[GrowingInsight], None] | None = None
        self._on_new_insight: Callable[[GrowingInsight], None] | None = None

        # Notification service integration
        self._notifications_service: "NotificationsService" | None = None
        self._user_resolver: Callable[[int], int | None] | None = None

        logger.info("ContinuousMonitoringService initialized")

    def start_monitoring(self, unit_ids: list[int] | None = None):
        """
        ...
        self._monitoring_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="ContinuousMonitoring"
        )
    ...
    def get_insights(self, unit_id: int, limit: int = 10, min_level: AlertLevel | None = None) -> list[GrowingInsight]:
        ...
        if min_level:
            ...
            insights = [i for i in insights if level_priority[i.alert_level] >= min_priority]
    ...
    def _monitor_unit(self, unit_id: int):
        ...
        disease_insights = self._analyze_disease_risk(unit_id, plant_type, growth_stage, current_conditions)
        ...
        # 3. Growth Progress
        growth_insights = self._analyze_growth_progress(unit_id, plant_type, growth_stage, current_conditions)
        ...
        env_health_insights = self._analyze_environmental_health(unit_id, plant_type, growth_stage, current_conditions)
        insights.extend(env_health_insights)

        # 6. Consolidated Recommendations
        rec_insights = self._analyze_recommendations(unit_id, plant_type, growth_stage, current_conditions)
        ...
        try:
            ...
            for risk in risks:
                if risk.risk_level.value in ["high", "critical"]:
                    alert_level = AlertLevel.CRITICAL if risk.risk_level.value == "critical" else AlertLevel.WARNING
        ...
        *,
        ...
        try:
            ...
            if recommendations["priority"] in ["high", "critical"]:
                alert_level = AlertLevel.CRITICAL if recommendations["priority"] == "critical" else AlertLevel.WARNING
    ...
    def _analyze_growth_progress(
        ...
        try:
            ...
            transition = self.growth_predictor.analyze_stage_transition(
                current_stage=growth_stage, days_in_stage=days_in_stage, actual_conditions=current_conditions
            )
    ...
    def _analyze_trends(self, unit_id: int, current_conditions: dict[str, float]) -> list[GrowingInsight]:
            ...
            for metric in ["temperature", "humidity", "soil_moisture"]:
                ...
                if abs(slope * len(values)) > trend_threshold[metric]:
                    ...
                    insight = GrowingInsight(
                        ...
                        description=f"{metric.replace('_', ' ').title()} is {direction} " + f"(current: {current:.1f})",
                        data={
                            "metric": metric,
                        ...
                        action_items=[
                            ...
                            "Consider adjusting controls if trend continues",
                        ],
                        timestamp=datetime.now(),
                        expires_at=datetime.now() + timedelta(hours=24),
                    )
    ...
    def set_callbacks(
        self,
        on_critical_alert: Callable[[GrowingInsight], None] | None = None,
        on_new_insight: Callable[[GrowingInsight], None] | None = None,
    ):
    ...
    def set_notification_service(
        self, notifications_service: "NotificationsService", user_resolver: Callable[[int], int | None]
    ) -> None:
        """
    ...
    def _analyze_environmental_health(
        ...
        try:
            ...
            if score and score.overall_score < 60:
                alert_level = AlertLevel.CRITICAL if score.overall_score < 30 else AlertLevel.WARNING
                insights.append(
                    GrowingInsight(
                        unit_id=unit_id,
                        insight_type="alert",
