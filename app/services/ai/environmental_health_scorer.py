...
class EnvironmentalLeafHealthScorer:
    ...
    def __init__(
        self, threshold_service: ThresholdService | None = None, analytics_repo: AnalyticsRepository | None = None
    ):
        """
    ...
    def score_current_health(
        ...
        try:
            ...
            temp_score = self._score_temperature(current_conditions.get("temperature", 22.0), thresholds)

            humidity_score = self._score_humidity(current_conditions.get("humidity", 60.0), thresholds)

            vpd_score = self._score_vpd(current_conditions.get("vpd", 1.0), growth_stage)

            moisture_score = self._score_soil_moisture(current_conditions.get("soil_moisture", 60.0), thresholds)
            ...
            disease_risk = self._assess_disease_risk(current_conditions, overall)
    ...
    def _score_temperature(self, temp: float, thresholds: dict[str, Any]) -> float:
    ...
    def _score_humidity(self, humidity: float, thresholds: dict[str, Any]) -> float:
    ...
    def _score_vpd(self, vpd: float, growth_stage: str | None) -> float:
    ...
    def _score_soil_moisture(self, moisture: float, thresholds: dict[str, Any]) -> float:
    ...
    def _calculate_stress_score(
        ...
        try:
            ...
            sensor_df = self.analytics_repo.get_sensor_time_series(
                unit_id, start_time.isoformat(), end_time.isoformat(), interval_hours=1
            )
            ...
            if "temperature" in sensor_df.columns:
                temp_stress_hours = ((sensor_df["temperature"] < temp_lo) | (sensor_df["temperature"] > temp_hi)).sum()

            if "humidity" in sensor_df.columns:
                humidity_stress_hours = ((sensor_df["humidity"] > hum_hi) | (sensor_df["humidity"] < hum_lo)).sum()

            if "soil_moisture" in sensor_df.columns:
                moisture_stress_hours = (
                    (sensor_df["soil_moisture"] < moist_lo) | (sensor_df["soil_moisture"] > moist_hi)
                ).sum()
            ...
            total_stress_hours = temp_stress_hours + humidity_stress_hours + moisture_stress_hours
    ...
    def _assess_disease_risk(self, current: dict[str, float], health_score: float) -> RiskLevel:
        ...
        elif (humidity > 75 and vpd < 0.8) or health_score < 0.5:
    ...
    def _generate_recommendations(
        ...
        if abs(temp - optimal_temp) > 3:
            if temp < optimal_temp:
                recommendations.append(f"Increase temperature to {optimal_temp}°C (currently {temp:.1f}°C)")
            else:
                recommendations.append(f"Decrease temperature to {optimal_temp}°C (currently {temp:.1f}°C)")
        ...
        if abs(humidity - optimal_humidity) > 10:
            if humidity < optimal_humidity:
                recommendations.append(f"Increase humidity to {optimal_humidity}% (currently {humidity:.1f}%)")
            else:
                recommendations.append(f"Decrease humidity to {optimal_humidity}% (currently {humidity:.1f}%)")
