...
from app.utils.time import utc_now, coerce_datetime
...
class AnalyticsService:
    ...
    def __init__(
        ...
        growth_repository: Optional[GrowthRepository] = None,
        ...
        try:
            ...
            for date in sorted(daily_data.keys()):
                ...
                daily_costs.append({
                    ...
                    'cost': round(cost, 2),
                    'avg_power_watts': round(sum(data['power_readings']) / len(data['power_readings']), 2) if data['power_readings'] else 0.0
                })
            ...
            logger.info(f"📊 Energy cost trends for actuator {actuator_id}: {len(daily_costs)} days, ${total_cost:.2f} total")
    ...
    def get_comparative_energy_analysis(
        ...
        try:
            ...
            actuators = self.device_repo.list_actuators(unit_id=unit_id) if unit_id else self.device_repo.list_actuators()
    ...
    def get_actuators_analytics_overview(
        self,
        unit_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_environmental_dashboard_summary(
        self,
        unit_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_energy_dashboard_summary(
        self,
        unit_id: Optional[int] = None,
        days: int = 7
    ) -> Dict[str, Any]:
    ...
    def calculate_environmental_stability(
        ...
        try:
            ...
            sensors = self.device_repo.list_sensor_configs(unit_id=unit_id) if unit_id else self.device_repo.list_sensor_configs()
    ...
    def get_composite_efficiency_score(
        ...
        try:
            ...
            previous_overall_score = (
                previous_env_score * 0.40 +
                previous_energy_score * 0.30 +
                previous_automation_score * 0.30
            )
    ...
    def warm_cache(self, unit_ids: Optional[List[int]] = None) -> Dict[str, Any]:
        """
    ...
    def get_enriched_sensor_history(
        ...
        *,
        ...
        interval: Optional[str] = None,
        lux_threshold_override: Optional[float] = None,
        prefer_lux: bool = False,
        day_start_override: Optional[str] = None,
        day_end_override: Optional[str] = None,
