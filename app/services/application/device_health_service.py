...
class DeviceHealthService:
    ...
    def _get_sensor_unit_id(self, sensor_id: int) -> int | None:
    ...
    def _get_actuator_unit_id(self, actuator_id: int) -> int | None:
    ...
    def calibrate_sensor(
        self, sensor_id: int, reference_value: float, calibration_type: str = "linear"
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not unit_id:
                return {"success": False, "error": f"Sensor {sensor_id} not found", "error_type": "not_found"}

            # Read current sensor value from sensor service
            if not self.sensor_service:
                return {"success": False, "error": "Sensor service not available", "error_type": "service_unavailable"}
            ...
            self.calibration_service.add_calibration_point(
                sensor_id=sensor_id, measured_value=float(measured_value), reference_value=float(reference_value)
            )
    ...
    def get_sensor_health(self, sensor_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            last_reading = (
                health_dict.get("last_reading") or health_dict.get("last_reading_time") or health_dict.get("last_check")
            )
            ...
            success_reads = health_dict.get("successful_reads")
    ...
    def check_sensor_anomalies(self, sensor_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            reading = self.sensor_service.read_sensor(sensor_id)
            if not reading:
                return {"success": False, "error": f"Failed to read sensor {sensor_id}", "error_type": "read_error"}
                    ...
                    if std_dev > 0.0:
                        ...
                        if self.alert_service:
                            try:
                                ...
                                try:
                                    logger.info(
                                        "Creating alert for sensor anomaly: sensor_id=%s, z_score=%.2f",
                                        sensor_id,
                                        z_score,
                                    )
    ...
    def log_actuator_anomaly(
        self, actuator_id: int, anomaly_type: str, severity: str, details: dict[str, Any] | None = None
    ) -> int | None:
        """
        ...
        try:
            ...
            if self.alert_service:
                try:
                    ...
                    actuator_name = (
                        actuator_info.get("name", f"Actuator {actuator_id}")
                        if actuator_info
                        else f"Actuator {actuator_id}"
                    )
    ...
    def save_actuator_calibration(
        self, actuator_id: int, calibration_type: str, calibration_data: dict[str, Any]
    ) -> int | None:
        """
    ...
    def check_all_devices_health_and_alert(self, unit_id: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            all_sensor_ids = [s.get("sensor_id") for s in sensors if s.get("sensor_id")]
    ...
    def _normalize_raw_health(self, sensor_id: int, raw_health: Any) -> dict[str, Any]:
    ...
    def evaluate_sensor_status(
        self, value: float | None, sensor_type: str, thresholds: dict[str, Any] | None = None
    ) -> str:
        """
    ...
    def calculate_system_health(
        self,
        vpd_status: str | None = None,
        plant_health_avg: float | None = None,
        ...
        return {"health_score": health_score, "status": status, "factors": factor_details}
