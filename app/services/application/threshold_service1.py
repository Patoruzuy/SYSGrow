...
class ThresholdService:
    ...
    def __init__(
        ...
        self._invalidate_unit_cache_callback: Optional[Callable[[int], None]] = None
    ...
    def subscribe_to_events(self) -> None:
        ...
        if self.event_bus:
            self.event_bus.subscribe(
                RuntimeEvent.THRESHOLDS_PERSIST, 
                self._handle_thresholds_persist
            )
    ...
    def get_environment_thresholds(self) -> Optional[Dict[str, Any]]:
        """
        Fetch global environment thresholds (legacy Settings table).

        Note: Global settings are deprecated; unit thresholds now live in GrowthUnits.
        """
        if not self.settings_repo:
            return None
        data = self.settings_repo.get_environment_thresholds()
    ...
    def update_environment_thresholds(
        ...
        *,
        ...
        """
        Persist global environment thresholds (legacy Settings table).

        Soil moisture is managed per-plant and excluded here.
        """
        if not self.settings_repo:
            logger.warning("SettingsRepository not available; cannot persist environment thresholds")
            return None
        current = self.settings_repo.get_environment_thresholds() or {}
        soil_moisture_threshold = current.get("soil_moisture_threshold", 0.0)
        self.settings_repo.save_environment_thresholds(
        ...
        try:
            ...
            if not plants:
                ...
                ranges = {
                    ...
                    "soil_moisture": {"min": generic.soil_moisture, "max": generic.soil_moisture, "optimal": generic.soil_moisture},
            ...
            if growth_stage:
                matched = [
                    stage
                    for stage in growth_stages
                    if str(stage.get("stage", "")).lower() == str(growth_stage).lower()
                ]
            ...
            soil_moisture_min = float(moisture_range.get("min")) if moisture_range.get("min") is not None else thresholds.soil_moisture
            soil_moisture_max = float(moisture_range.get("max")) if moisture_range.get("max") is not None else thresholds.soil_moisture
    ...
    def is_within_optimal_range(
        ...
        try:
            ...
            if 'soil_moisture' in current_conditions:
                ...
                results['soil_moisture'] = abs(current_conditions['soil_moisture'] - thresholds.soil_moisture) <= tolerance
