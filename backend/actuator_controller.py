...
class GIORelayActuator(Actuator):
    ...
    def __init__(self, actuator_id, device, pin=None, init_delay=1):
        ...
        try:
            self.relay = GPIORelay(device, pin)
        except ValueError:
...
class WiFiRelayActuator(Actuator):
    ...
    def __init__(self, actuator_id, device, ip=None):
        """
        ...
        self.state = 'off'
        try:
            self.relay = WiFiRelay(device, ip)
        except ValueError:
            raise ValueError("No valid relay configuration provided.")
...
class ZigbeeRelayActuator(Actuator):
    """Handles wireless relay actuators connected via Zigbee."""

    def __init__(self, actuator_id, device, zigbee_channel, mqtt_broker="localhost", mqtt_port=1883):
        """
        Initializes the ZigbeeRelayActuator with the given Zigbee address.

        Args:
            actuator_id (int): Unique ID for the actuator.
            device (str): Device name
            zigbee_channel (str): The relay channel.
            mqtt_broker (str, optional): MQTT broker (default: localhost).
            mqtt_port (int, optional): MQTT broker port (default: 1883).
        """
        self.id = actuator_id
        self.device = device
        ...
        self.state = 'off'
        try:
            self.relay = ZigbeeRelay(device, zigbee_channel, mqtt_broker, mqtt_port)
        except ValueError:
            raise ValueError("No valid relay configuration provided.")

    def activate(self):
        """
        Sends a Zigbee command to activate the relay.
        """
        self.relay.turn_on()
        self.state = 'on'
        print(f"Zigbee Relay activated for device {self.device}")

    def deactivate(self):
        """
        Sends a Zigbee command to deactivate the relay.
        """
        self.relay.turn_off()
        self.state = 'off'
        print(f"Zigbee Relay deactivated for device {self.device}")
...
class ActuatorFactory:
    ...
    def create_actuator(actuator_type, **kwargs):
        ...
        if actuator_type == "GPIO":
            ...
            return ZigbeeRelayActuator(
                actuator_id=kwargs.get("actuator_id"),
                device=kwargs.get("device"),
                zigbee_topic=kwargs.get("zigbee_topic"),
...
class ActuatorController:
    ...
    def deactivate_actuator(self, device):
        ...
        if device in self.actuators:
            ...
            logging.info(f"Deactivate name: {device}")
        else:
