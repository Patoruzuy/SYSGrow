...
@safe_route("Failed to get schedules")
def get_schedules(unit_id: int) -> Response:
    """
    ...
    """
    logger.info("[v3] Getting schedules for unit %s", unit_id)
...
@safe_route("Failed to get schedule")
def get_schedule(unit_id: int, schedule_id: int) -> Response:
    """Get a specific schedule by ID."""
    logger.info("[v3] Getting schedule %s for unit %s", schedule_id, unit_id)
    sched_service = _scheduling_service()
...
@safe_route("Failed to create schedule")
def create_schedule(unit_id: int) -> Response:
    """
    Create a new schedule for a growth unit.

    Supports multiple schedules per device type.
    """
    logger.info("Creating schedule for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
    ...
    if str(device_type or "").lower() == "light" and str(schedule_type or "").lower() == "automatic" and not end_time:
    ...
    logger.info("Created schedule %s for unit %s", created.schedule_id, unit_id)
    return _success({"schedule": created.to_dict()}, status=201)


@growth_api.put("/v3/units/<int:unit_id>/schedules/<int:schedule_id>")
@safe_route("Failed to update schedule")
def update_schedule(unit_id: int, schedule_id: int) -> Response:
    """Update an existing schedule."""
    logger.info("Updating schedule %s for unit %s", schedule_id, unit_id)
    sched_service = _scheduling_service()
...
@safe_route("Failed to toggle schedule")
def toggle_schedule(unit_id: int, schedule_id: int) -> Response:
    """Enable or disable a schedule without deleting it."""
    logger.info("Toggling schedule %s for unit %s", schedule_id, unit_id)
    sched_service = _scheduling_service()
...
@safe_route("Failed to bulk update schedules")
def bulk_update_schedules(unit_id: int) -> Response:
    ...
    """
    logger.info("[v3] Bulk updating schedules for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to delete schedule")
def delete_schedule(unit_id: int, schedule_id: int) -> Response:
    """Delete a schedule permanently."""
    logger.info("[v3] Deleting schedule %s for unit %s", schedule_id, unit_id)
    sched_service = _scheduling_service()
...
@safe_route("Failed to get active schedules")
def get_active_schedules(unit_id: int) -> Response:
    """Get list of currently active schedules based on time and day."""
    logger.info("[v3] Getting active schedules for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to get schedule summary")
def get_schedule_summary(unit_id: int) -> Response:
    """Get summary of all schedules for a unit."""
    logger.info("[v3] Getting schedule summary for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to get schedule preview")
def preview_schedules(unit_id: int) -> Response:
    """
    ...
    Query params:
        ...
        device_type: Filter by device type (optional)
    """
    logger.info("[v3] Getting schedule preview for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to get schedule history")
def get_schedule_history(unit_id: int) -> Response:
    """
    ...
    """
    logger.info("[v3] Getting schedule history for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to get execution log")
def get_schedule_execution_log(schedule_id: int) -> Response:
    """
    ...
    Query params:
        limit: Maximum records to return (default 100, max 1000)
    """
    logger.info("[v3] Getting execution log for schedule %s", schedule_id)
    sched_service = _scheduling_service()
...
@safe_route("Failed to detect schedule conflicts")
def detect_schedule_conflicts(unit_id: int) -> Response:
    """
    ...
    Query params:
        device_type: Filter by device type (optional)
    """
    logger.info("[v3] Detecting schedule conflicts for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to auto-generate schedules")
def auto_generate_schedules(unit_id: int) -> Response:
    """
    ...
    """
    logger.info("Auto-generating schedules for unit %s", unit_id)
    sched_service = _scheduling_service()
    growth_service = _service()
...
@safe_route("Failed to get schedule templates")
def get_schedule_templates(unit_id: int) -> Response:
    """
    ...
    """
    logger.info("[v3] Getting schedule templates for unit %s", unit_id)
    _scheduling_service()
    growth_service = _service()
