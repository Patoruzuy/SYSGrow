...
from app.utils.http import error_response, safe_route

harvest_bp = Blueprint("harvest", __name__)


@harvest_bp.route("/api/v1/plants/<int:plant_id>/harvest", methods=["POST"])
@safe_route("Failed to create harvest report")
def create_harvest_report(plant_id) -> Response:
    """
    ...
    try:
        ...
        try:
            ...
            response = jsonify(
                {"ok": False, "data": None, "error": {"message": "Invalid request", "details": ve.errors()}}
            )
            response.status_code = 400
            return response
        ...
        return error_response("Invalid harvest request", 400)


@harvest_bp.route("/api/v1/harvests/<int:harvest_id>", methods=["GET"])
@safe_route("Failed to retrieve harvest report")
def get_harvest_report(harvest_id) -> Response:
    """
...
@harvest_bp.route("/api/v1/harvests", methods=["GET"])
@safe_route("Failed to list harvest reports")
def list_harvest_reports() -> Response:
    """
    ...
    response = jsonify(
        {
            "ok": True,
            "data": {"total": total, "limit": limit, "offset": offset, "harvests": paginated},
            "error": None,
        }
    )
    response.status_code = 200
...
@harvest_bp.route("/api/v1/units/<int:unit_id>/plants", methods=["GET"])
@safe_route("Failed to get plants")
def get_plants_for_unit(unit_id) -> Response:
    """
...
@harvest_bp.route("/api/v1/plants/<int:plant_id>", methods=["DELETE"])
@safe_route("Failed to delete plant")
def delete_plant(plant_id) -> Response:
    """
        ...
        response = jsonify(
            {
                ...
                "data": None,
                "error": {
                    "message": "This endpoint only supports post-harvest deletion. Use harvested=true parameter."
                },
            }
        )
        response.status_code = 400
    ...
    result = service.cleanup_after_harvest(plant_id=plant_id, delete_plant_data=True)
...
@harvest_bp.route("/api/v1/units/<int:unit_id>/harvest-stats", methods=["GET"])
@safe_route("Failed to get harvest statistics")
def get_unit_harvest_stats(unit_id) -> Response:
    """
    ...
    response = jsonify(
        {"ok": True, "data": {"unit_id": unit_id, "harvest_count": len(stats), "statistics": stats}, "error": None}
    )
    response.status_code = 200
    return response


@harvest_bp.route("/api/v1/units/<int:unit_id>/growth-cycles/compare", methods=["GET"])
@safe_route("Failed to compare growth cycles")
def compare_growth_cycles(unit_id) -> Response:
    """
    ...
    if not analytics_repo:
        return jsonify({"ok": False, "data": None, "error": {"message": "Analytics repository not available"}}), 503
    ...
    if comparison.get("error"):
        return jsonify({"ok": False, "data": None, "error": {"message": comparison["error"]}}), 500

    return jsonify({"ok": True, "data": comparison, "error": None}), 200


@harvest_bp.route("/api/v1/harvests/compare", methods=["GET"])
@safe_route("Failed to compare harvests")
def compare_specific_harvests() -> Response:
    """
    ...
    try:
        ...
        return jsonify({"ok": False, "data": None, "error": {"message": "Invalid harvest IDs format"}}), 400

    if len(harvest_ids) < 2:
        return jsonify(
            {"ok": False, "data": None, "error": {"message": "At least 2 harvest IDs are required for comparison"}}
        ), 400

    if len(harvest_ids) > 10:
        return jsonify(
            {"ok": False, "data": None, "error": {"message": "Maximum 10 harvests can be compared at once"}}
        ), 400
...
@harvest_bp.route("/api/v1/plants/types/<plant_type>/performance", methods=["GET"])
@safe_route("Failed to get plant type performance")
def get_plant_type_performance(plant_type) -> Response:
    """
    ...
    if performance.get("error"):
        return jsonify({"ok": False, "data": None, "error": {"message": performance["error"]}}), 500

    return jsonify({"ok": True, "data": performance, "error": None}), 200


@harvest_bp.route("/api/v1/units/<int:unit_id>/sensor-summaries", methods=["GET"])
@safe_route("Failed to get sensor summaries")
def get_unit_sensor_summaries(unit_id) -> Response:
    """
    ...
    if limit < 1 or limit > 2000:
        response = jsonify({"ok": False, "data": None, "error": {"message": "limit must be between 1 and 2000"}})
        response.status_code = 400
        return response

    container = get_container()
    analytics_service = getattr(container, "analytics_service", None)
    # Fallback to device_repo for backward compatibility
    device_repo = getattr(container, "device_repo", None)

    if analytics_service:
        summaries = analytics_service.get_sensor_summaries_for_unit(
            unit_id=unit_id,
        ...
        response = jsonify({"ok": False, "data": None, "error": {"message": "Analytics service not available"}})
...
@harvest_bp.route("/api/v1/units/<int:unit_id>/harvest-environment", methods=["GET"])
@safe_route("Failed to get environment statistics")
def get_harvest_environment_stats(unit_id) -> Response:
    """
    ...
    if not start_date or not end_date:
        response = jsonify({"ok": False, "data": None, "error": {"message": "start_date and end_date are required"}})
        response.status_code = 400
        return response
    ...
    if analytics_service:
        stats = analytics_service.get_sensor_summary_stats_for_harvest(
            unit_id=unit_id,
            start_date=start_date,
            end_date=end_date,
        )
...
@harvest_bp.route("/api/v1/health", methods=["GET"])
@safe_route("Health check failed", error_status=503)
def health_check() -> Response:
    ...
    response = jsonify(
        {
            "ok": True,
            "data": {"status": HealthLevel.HEALTHY.value, "service": "harvest", "version": "1.0.0"},
            "error": None,
        }
    )
    response.status_code = 200
