...
from app.utils.http import error_response, safe_error, safe_route, success_response
...
def get_request(request_id: int) -> Response:
    ...
    return jsonify({"ok": True, "data": req, "error": None})
...
def predict_manual_next_irrigation(plant_id: int) -> Response:
    ...
    try:
        ...
        return safe_error(exc, 503, context="load plant irrigation model service")
...
def start_pump_calibration() -> Response:
    ...
    try:
        ...
        return error_response("Pump calibration service not available", 503)
...
def complete_pump_calibration(actuator_id: int) -> Response:
    ...
    try:
        ...
        return jsonify({"ok": True, "data": response_data, "error": None})
    except ValueError as e:
        return jsonify({"ok": False, "error": {"message": str(e)}}), 400
