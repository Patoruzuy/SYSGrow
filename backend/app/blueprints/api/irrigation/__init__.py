...
def get_request(request_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Error getting request {request_id}: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def delay_request(request_id: int):
    """
    ...
    try:
        ...
        current_app.logger.error(f"Error delaying request {request_id}: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def cancel_request(request_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Error cancelling request {request_id}: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500


@irrigation_bp.route("/requests/<int:request_id>/feedback", methods=["POST"])
def submit_feedback(request_id: int):
    """
    ...
    try:
        ...
        current_app.logger.error(f"Error submitting feedback for request {request_id}: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def log_manual_irrigation():
    ...
    try:
        ...
        current_app.logger.error(f"Error logging manual irrigation: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def get_execution_logs(unit_id: int):
    """Fetch irrigation execution logs for diagnostics."""
    try:
        container = get_container()
        repo = IrrigationWorkflowRepository(container.database)

        limit = request.args.get("limit", 200, type=int)
        plant_id = request.args.get("plant_id", type=int)
        start_ts, end_ts = _resolve_time_window()

        logs = repo.get_execution_logs_for_unit(
            unit_id,
            start_ts=start_ts,
            end_ts=end_ts,
            limit=limit,
        ...
        return error_response("Failed to get execution logs", 500, details={"message": str(e)})


@irrigation_bp.route("/eligibility/<int:unit_id>", methods=["GET"])
def get_eligibility_traces(unit_id: int):
    ...
    try:
        ...
        traces = repo.get_eligibility_traces_for_unit(
            unit_id,
            start_ts=start_ts,
            end_ts=end_ts,
            limit=limit,
        ...
        current_app.logger.error(f"Error getting eligibility traces: {e}")
        return error_response("Failed to get eligibility traces", 500, details={"message": str(e)})


@irrigation_bp.route("/manual/<int:unit_id>", methods=["GET"])
def get_manual_irrigation_history(unit_id: int):
    ...
    try:
        ...
        logs = repo.get_manual_logs_for_unit(
            unit_id,
            start_ts=start_ts,
            end_ts=end_ts,
            limit=limit,
        ...
        current_app.logger.error(f"Error getting manual irrigation history: {e}")
        return error_response("Failed to get manual irrigation history", 500, details={"message": str(e)})


@irrigation_bp.route("/manual/predict/<int:plant_id>", methods=["GET"])
def predict_manual_next_irrigation(plant_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Error predicting next manual irrigation: {e}")
        return error_response("Failed to predict next irrigation", 500, details={"message": str(e)})
...
def update_config(unit_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Error updating config for unit {unit_id}: {e}")
        return error_response(
            "Failed to update irrigation config",
            status=500,
            details={"unit_id": unit_id, "reason": str(e)},
        )
...
def handle_action(request_id: int):
    """
...
def start_pump_calibration():
    ...
    try:
        ...
        current_app.logger.error(f"Error starting pump calibration: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def complete_pump_calibration(actuator_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Error completing pump calibration: {e}")
        return jsonify({"ok": False, "error": {"message": str(e)}}), 500
...
def get_pump_calibration_history(actuator_id: int):
    """
...
def get_irrigation_recommendations(plant_id: int):
    """
...
def calculate_irrigation(plant_id: int):
    """
...
def send_recommendation_notification(plant_id: int):
    """
