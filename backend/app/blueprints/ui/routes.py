...
def _ensure_selected_unit():
    ...
    unit_ids: list[int] = []
    for unit in units:
        candidate = unit.get("unit_id") if isinstance(unit, dict) else getattr(unit, "unit_id", None)
        try:
            parsed = int(candidate)
        except (TypeError, ValueError):
            continue
        unit_ids.append(parsed)

    if not unit_ids:
        logger.warning("Unable to resolve valid unit_id values from units list")
        return None, units

    selected_raw = session.get("selected_unit")
    selected_id = None
    if selected_raw is not None:
        try:
            selected_id = int(selected_raw)
        except (TypeError, ValueError):
            logger.warning("Selected unit id is invalid: %s", selected_raw)
            selected_id = None

    if selected_id not in unit_ids:
        selected_id = unit_ids[0]
        session["selected_unit"] = selected_id
        session.modified = True

    return selected_id, units
