...
class AppConfig:
    ...
    actuator_state_retention_days: int = field(default_factory=lambda: _env_int("SYSGROW_ACTUATOR_STATE_RETENTION_DAYS", 90))
    ...
    api_health_window_size: int = field(
        default_factory=lambda: _env_int("SYSGROW_API_HEALTH_WINDOW_SIZE", 100)
    )
    api_health_min_samples: int = field(
        default_factory=lambda: _env_int("SYSGROW_API_HEALTH_MIN_SAMPLES", 10)
    )
    ...
    audit_log_path: str = field(
        default_factory=lambda: os.getenv("SYSGROW_AUDIT_LOG_PATH", "logs/audit.log")
    )
    ...
    training_data_path: str = field(
        default_factory=lambda: os.getenv("TRAINING_DATA_PATH", "data/training")
    )
    models_path: str = field(
        default_factory=lambda: os.getenv("MODELS_PATH", "models")
    )
    ...
    retraining_check_interval: int = field(
        default_factory=lambda: int(os.getenv("RETRAINING_CHECK_INTERVAL", "3600"))
    )
    
    # Model Configuration
    model_min_training_samples: int = field(
        default_factory=lambda: int(os.getenv("MODEL_MIN_TRAINING_SAMPLES", "100"))
    )
    ...
    max_concurrent_predictions: int = field(
        default_factory=lambda: int(os.getenv("MAX_CONCURRENT_PREDICTIONS", "3"))
    )
    ...
    ai_log_predictions: bool = field(
        default_factory=lambda: _env_bool("AI_LOG_PREDICTIONS", True)
    )
    ai_log_training_details: bool = field(
        default_factory=lambda: _env_bool("AI_LOG_TRAINING_DETAILS", True)
    )
    ai_metrics_export_interval: int = field(
        default_factory=lambda: _env_int("AI_METRICS_EXPORT_INTERVAL", 3600)
    )
    ...
    enable_drift_detection: bool = field(
        default_factory=lambda: _env_bool("ENABLE_DRIFT_DETECTION", True)
    )
    ...
    monitoring_alert_threshold: str = field(
        default_factory=lambda: os.getenv("MONITORING_ALERT_THRESHOLD", "warning")
    )
    ...
    training_data_retention_days: int = field(
        default_factory=lambda: _env_int("TRAINING_DATA_RETENTION_DAYS", 365)
    )
    
    # Model Configuration (Extended)
    model_validation_split: float = field(
        default_factory=lambda: float(os.getenv("MODEL_VALIDATION_SPLIT", "0.2"))
    )
    model_cross_validation_folds: int = field(
        default_factory=lambda: _env_int("MODEL_CROSS_VALIDATION_FOLDS", 5)
    )
    model_cache_ttl: int = field(
        default_factory=lambda: _env_int("MODEL_CACHE_TTL", 300)
    )
    ...
    retraining_max_concurrent_jobs: int = field(
        default_factory=lambda: _env_int("RETRAINING_MAX_CONCURRENT_JOBS", 1)
    )
    retraining_schedule_climate: str = field(
        default_factory=lambda: os.getenv("RETRAINING_SCHEDULE_CLIMATE", "weekly")
    )
    retraining_schedule_disease: str = field(
        default_factory=lambda: os.getenv("RETRAINING_SCHEDULE_DISEASE", "on_drift")
    )
    retraining_schedule_growth: str = field(
        default_factory=lambda: os.getenv("RETRAINING_SCHEDULE_GROWTH", "monthly")
    )
    
    # Drift Detection Configuration
    drift_detection_window_size: int = field(
        default_factory=lambda: _env_int("DRIFT_DETECTION_WINDOW_SIZE", 100)
    )
    ...
    prediction_timeout_seconds: int = field(
        default_factory=lambda: _env_int("PREDICTION_TIMEOUT_SECONDS", 30)
    )
    enable_gpu_acceleration: bool = field(
        default_factory=lambda: _env_bool("ENABLE_GPU_ACCELERATION", False)
    )
    ...
    notify_climate_issues: bool = field(
        default_factory=lambda: _env_bool("NOTIFY_CLIMATE_ISSUES", True)
    )
    ...
    cv_model_type: str = field(
        default_factory=lambda: os.getenv("CV_MODEL_TYPE", "mobilenet_v2")
    )
    cv_inference_device: str = field(
        default_factory=lambda: os.getenv("CV_INFERENCE_DEVICE", "cpu")
    )
    cv_confidence_threshold: float = field(
        default_factory=lambda: float(os.getenv("CV_CONFIDENCE_THRESHOLD", "0.7"))
    )
    cv_capture_interval: int = field(
        default_factory=lambda: _env_int("CV_CAPTURE_INTERVAL", 21600)
    )
    ...
    ab_testing_min_samples: int = field(
        default_factory=lambda: _env_int("AB_TESTING_MIN_SAMPLES", 100)
    )
...
def validate_ai_config(config: AppConfig) -> list[str]:
    ...
    if config.max_concurrent_predictions > 3:
        warnings.append(
            f"Concurrent predictions ({config.max_concurrent_predictions}) may overload Raspberry Pi. "
            "Recommended: 1-3"
        )
