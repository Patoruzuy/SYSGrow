...
class SensorRegistry:
    ...
    def register_protocol_adapter(
        ...
        logger.info(
            f"Registered protocol '{protocol}' "
            f"with adapter {adapter_class.__name__}"
        )
    ...
    def register_sensor_type(
        ...
        sensor_type: str,
        adapter_class: Type[ISensorAdapter],
        processors: Optional[List[Type[IDataProcessor]]] = None,
    ...
    def get_adapter_class_by_protocol(self, protocol: str) -> Optional[Type[ISensorAdapter]]:
        """
    ...
    def get_adapter_class(self, sensor_type: str) -> Optional[Type[ISensorAdapter]]:
        """
    ...
    def auto_register_protocol_adapters(
        self,
        adapter_mapping: Dict[str, Type[ISensorAdapter]]
    ):
        """
...
_global_registry: Optional[SensorRegistry] = None
...
def _initialize_default_registry(registry: SensorRegistry):
    ...
    registry.register_protocol_adapter(
        Protocol.GPIO.value,
        GPIOAdapter,
        capabilities=['wired', 'direct_gpio']
    )
    ...
    registry.register_protocol_adapter(
        Protocol.MQTT.value,
        SYSGrowAdapter,
        capabilities=['wifi', 'mqtt', 'bidirectional', 'ota_update']
    )
    
    registry.register_protocol_adapter(
        Protocol.WIRELESS.value,
        SYSGrowAdapter,
        capabilities=['wifi', 'mqtt', 'bidirectional', 'ota_update']
    )
    ...
    registry.register_protocol_adapter(
        Protocol.HTTP.value,
        WiFiAdapter,
        capabilities=['wifi', 'http', 'direct_connection']
    )
    
    registry.register_protocol_adapter(
        Protocol.ZIGBEE.value,
        ZigbeeAdapter,
        capabilities=['battery_powered', 'low_power', 'mesh']
    )
    ...
    registry.register_protocol_adapter(
        Protocol.MODBUS.value,
        ModbusAdapter,
        capabilities=['industrial', 'wired', 'high_accuracy']
    )
