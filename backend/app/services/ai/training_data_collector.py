...
class TrainingExample:
    ...
    growth_success: bool | None  # Did plant thrive?
    quality_rating: int | None  # 1-5 scale
    yield_amount: float | None  # For harvest predictions
...
class TrainingDataCollector:
    ...
    def collect_disease_training_data(self, days_back: int = 30, min_examples_per_class: int = 50):
        """
        ...
        for obs in observations:
            try:
                ...
                sensor_df = self.training_data_repo.get_sensor_time_series(
                    obs["unit_id"], sensor_start.isoformat(), obs_time.isoformat(), interval_hours=1
                )

                # Skip if insufficient sensor data
                if len(sensor_df) < self.min_sensor_readings:
                    logger.debug("Skipping observation: insufficient sensor data")
                    continue

                # Extract features
                features = self.feature_engineer.create_disease_features(
                    sensor_data=sensor_df, growth_stage=obs.get("growth_stage", "Vegetative"), current_time=obs_time
                )
        ...
        # Balance dataset
        df_balanced = self._balance_dataset(df, label_column="disease_type", min_per_class=min_examples_per_class)
    ...
    def collect_climate_training_data(self, days_back: int = 60):
        """
        ...
        adjustments = self.training_data_repo.get_user_adjustments(start_date.isoformat(), end_date.isoformat())
        ...
        for adj in adjustments:
            try:
                ...
                sensor_df = self.training_data_repo.get_sensor_time_series(
                    adj["unit_id"], before_start.isoformat(), adj_time.isoformat(), interval_hours=1
                )
    ...
    def collect_growth_outcome_data(self, days_back: int = 180):
        """
        ...
        grow_cycles = self.training_data_repo.get_completed_grows(start_date.isoformat(), end_date.isoformat())
        ...
        for cycle in grow_cycles:
            try:
                ...
                for stage in stages:
                    ...
                    sensor_df = self.training_data_repo.get_sensor_time_series(
                        cycle["unit_id"], stage_start, stage_end, interval_hours=6
                    )
    ...
    def validate_training_data(self, df, data_type: str) -> tuple[bool, list[str]]:
        """
    ...
    def _balance_dataset(self, df, label_column: str, min_per_class: int):
        """
            ...
            if class_counts.min() < min_per_class:
                ...
                for label in class_counts.index:
                    ...
                    if len(class_df) < min_per_class:
                        ...
                        oversampled = class_df.sample(n=min_per_class, replace=True, random_state=42)
