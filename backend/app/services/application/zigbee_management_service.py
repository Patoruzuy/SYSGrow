...
class ZigbeeManagementService:
    ...
    def __init__(self, mqtt_client: Any, bridge_topic: str = "zigbee2mqtt"):
        ...
        """
        self._mqtt_wrapper: MQTTClientWrapper | None = (
            mqtt_client if isinstance(mqtt_client, MQTTClientWrapper) else None
        )
        ...
        self.last_rename_response: dict | None = None
    ...
    def _subscribe_to_topics(self) -> None:
        ...
        except ZIGBEE_RECOVERABLE_ERRORS as e:
            ...
            if friendly_name in self.state_callbacks:
                for callback in self.state_callbacks[friendly_name]:
                    try:
                        ...
                        # User-registered callbacks can raise arbitrary exceptions.
    ...
    def _process_device(self, device_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if not is_new_device:
                ...
                logger.debug(
                    f"Device {ieee_address} already registered as '{existing_device.friendly_name}', skipping..."
                )
            ...
            if is_new_device:
                for callback in self.discovery_callbacks:
                    try:
                        ...
                        # Discovery callbacks are injected and may fail independently.
    ...
    def get_device_by_ieee(self, ieee_address: str) -> DiscoveredDevice | None:
        """Get device by IEEE address"""
        return self.discovered_devices.get(ieee_address)

    def get_device_by_friendly_name(self, friendly_name: str) -> DiscoveredDevice | None:
    ...
    def get_device_state(self, friendly_name: str) -> dict[str, Any] | None:
        """Get current state of a device"""
        return self.device_states.get(friendly_name)

    def send_command(self, friendly_name: str, command: dict[str, Any]) -> bool:
        """
    ...
    def permit_device_join(self, time: int = 254, device_type: str | None = None) -> bool:
        """
    ...
    def rename_device(self, ieee_address: str, new_name: str, timeout: float = 3.0) -> dict[str, Any]:
        ...
        """
        payload = {"from": str(ieee_address), "to": str(new_name), "homeassistant_rename": False}
