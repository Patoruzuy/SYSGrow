...
from app.domain.system import SystemHealthStatus, SystemHealthLevel, SystemHealthReport
...
class SystemHealthService:
    ...
    def __init__(
        ...
        self._last_sensor_report: Optional[SystemHealthReport] = None
        ...
        self._api_health_error_rate_degraded = float(
            getattr(config, "api_health_error_rate_degraded", 0.10)
        )
        self._api_health_error_rate_offline = float(
            getattr(config, "api_health_error_rate_offline", 0.50)
        )
        self._api_health_avg_response_time_ms = int(
            getattr(config, "api_health_avg_response_time_ms", 2000)
        )
        self._api_health_slow_request_ms = int(
            getattr(config, "api_health_slow_request_ms", 1000)
        )
    ...
    def _system_startup(self) -> None:
        ...
        try:
            ...
            self.uptime_start = datetime.fromtimestamp(start_time, tz=timezone.utc)
            logger.info("System uptime tracking initialized.")
        except Exception as e:
            logger.warning(f"Failed to calculate server uptime: {e}")
    ...
    def register_sensor(self, sensor):
        ...
        logger.info(f"Registered sensor {sensor.id} for health monitoring")
    ...
    def unregister_sensor(self, sensor_id: int):
        ...
        if sensor_id in self._sensor_entities:
            ...
            logger.info(f"Unregistered sensor {sensor_id} from health monitoring")
    ...
    def generate_sensor_health_report(self) -> SystemHealthReport:
        ...
        system_health = self._determine_sensor_system_health(
            total, healthy, degraded, critical, offline
        )
    ...
    def get_last_sensor_report(self) -> Optional[SystemHealthReport]:
        """
    ...
    def get_sensor_health_trend(self, hours: int = 24) -> Dict:
        ...
        recent_reports = [
            r for r in self._sensor_health_history
    ...
    def update_health_status(self, system_id: int, level: SystemHealthLevel, message: str) -> None:
        ...
        logger.info(f"Updated health status for system {system_id}: {status.to_dict()}")
    
    def get_health_status(self, system_id: int) -> Optional[SystemHealthStatus]:
    ...
    def record_incident(self, system_id: int) -> None:
        ...
        if system_id in self.health_statuses:
            ...
            logger.info(f"Recorded incident for system {system_id}. Total incidents: {self.health_statuses[system_id].incident_count}")
        else:
            logger.warning(f"Attempted to record incident for unknown system {system_id}.")
    ...
    def get_comprehensive_health_report(self) -> Dict[str, Any]:
        ...
        if self._sensor_entities:
            try:
                ...
                logger.error(f"Failed to generate sensor health report: {e}")
    ...
    def check_and_alert_on_health_issues(self) -> List[int]:
        ...
        try:
            ...
            if created_alerts:
                logger.info(f"Created {len(created_alerts)} health alerts")
            
        except Exception as e:
            logger.error(f"Error checking health and creating alerts: {e}")
    ...
    def update_infrastructure_status(
        ...
        if api_status and api_status != self.api_status:
            ...
            status_changed = True
            logger.info(f"API status changed: {old_status} -> {api_status}")
        ...
        if db_status and db_status != self.db_status:
            ...
            status_changed = True
            logger.info(f"Database status changed: {old_status} -> {db_status}")
    ...
    def notify_health_change(self, system_id: int) -> None:
        ...
        if status:
            ...
            logger.info(f"Notified health change for system {system_id}.")
    ...
    def storage_usage_update(self, used: int, total: int) -> None:
        ...
        logger.info(f"Updated storage usage: {used}/{total} bytes.")
    ...
    def refresh_storage_usage(self, path: str = "/") -> Dict[str, Any]:
        ...
        try:
            ...
            self.storage_used = disk_usage.used
            self.storage_total = disk_usage.total
            
            usage_percent = disk_usage.percent
            
            logger.info(f"Storage refreshed: {usage_percent:.1f}% used ({disk_usage.used}/{disk_usage.total} bytes)")
            ...
            logger.error(f"Failed to refresh storage usage: {e}")
            return {
                "error": str(e),
                "total": 0,
                "used": 0,
    ...
    def check_database_health(self, db_handler: Optional[Any] = None) -> str:
        """
        ...
        try:
            ...
            if old_status != "connected":
                logger.info(f"Database health restored: {old_status} -> connected")
    ...
    def _update_api_status_from_metrics(self) -> None:
        ...
        if old_status != self.api_status:
            logger.warning(f"API status changed: {old_status} -> {self.api_status}")
    ...
    def perform_full_health_check(self, db_handler: Optional[Any] = None) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Health check complete. Overall status: {report['overall_status']}, score: {scores['overall']}")
