...
import logging
from mqtt.mqtt_broker_wrapper import MQTTClientWrapper
...
class WirelessRelay(RelayBase):
    ...
    def turn_on(self):
        ...
        try:
            ...
            self.event_bus.publish("relay_state_changed", {"device": self.device, "state": "on"}) # Publish Event
            logging.info(f"Turned on {self.connection_mode} relay for {self.device}")
        except Exception as e:
            logging.error(f"Error turning on relay {self.device}: {e}")
    ...
    def turn_off(self):
        ...
        try:
            if self.connection_mode == "Zigbee":
                ...
                raise ValueError(f"Invalid communication mode selected: {self.connection_mode}")
            self.event_bus.publish("relay_state_changed", {"device": self.device, "state": "off"}) # Publish Event
            logging.info(f"Turned off {self.connection_mode} relay for {self.device}")
        except Exception as e:
            logging.error(f"Error turning off relay {self.device}: {e}")
    ...
    def set_connection_mode(self, mode: str):
        ...
        logging.info(f"Switching connection mode to: {mode}")
    ...
    def _send_ble_command(self, state: str):
        ...
        if self.ble_client:
            try:
                ...
                logging.debug(f"BLE Command Sent: {state}")
            except Exception as e:
                raise Exception(f"Error sending BLE command: {e}") from e
        else:
            logging.warning("BLE client not initialized.")
    ...
    def update_wifi_credentials(self, mqtt_topic, encrypted_payload):
        ...
        logging.info(f"Sent new Wi-Fi credentials to topic {topic}")
    ...
    def update_esp32_mqtt_topic(self, new_topic):
        ...
        logging.info(f"Sent MQTT topic update command to ESP32-C6: {new_topic}")
