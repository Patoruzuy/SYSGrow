...
class MLUIManager {
    ...
    /**
     * Open modal with Bootstrap when available, fallback to class-based modal.
    ...
    _showModal(modal) {
        ...
        modal.classList.add('show', 'active', 'visible', 'is-open');
        modal.style.display = 'flex';
        document.body.classList.add('modal-open');
    }

    /**
     * Hide modal with Bootstrap when available, fallback to class-based modal.
     * @param {HTMLElement|null} modal
     */
    _hideModal(modal) {
        if (!modal) return;
        ...
        modal.classList.remove('show', 'active', 'visible', 'is-open');
    ...
    attachEventListeners() {
        ...
        // Bootstrap-like dismiss support when Bootstrap JS is not loaded.
        document.querySelectorAll('[data-bs-dismiss="modal"]').forEach(btn => {
            btn.addEventListener('click', (event) => {
                event.preventDefault();
                const modal = btn.closest('.modal');
                if (!modal) return;
                if (modal.id === 'features-modal') {
        ...
        // Backdrop click fallback for class-based modal mode.
        [this.elements.modelDetailsModal, this.elements.featuresModal].forEach(modal => {
            if (!modal) return;
                    ...
                    <li class="job-item">
                        ...
                        <div class="job-actions">
                            ...
                            <button class="btn btn-sm btn-secondary btn-toggle-job"
                                ...
                                <i class="fas ${job.enabled ? 'fa-pause' : 'fa-play'}" aria-hidden="true"></i>
    ...
    renderTrainingHistory(data) {
        ...
        if (data.history && data.history.length > 0) {
            let html = '<div class="table-responsive ml-training-history-wrap"><table class="table table-hover ml-training-history-table">';
            html += `
                <thead>
                    <tr>
                        <th>Model</th>
    ...
    async showModelDetails(modelName) {
        try {
            ...
            this._showModal(this.elements.modelDetailsModal);
        } catch (error) {
    ...
    async showFeaturesModal(modelName) {
        if (!this.elements.featuresModal) return;
        this._showModal(this.elements.featuresModal);
     ...
     */
    closeModal() {
        this._hideModal(this.elements.modelDetailsModal);
    }
     ...
     */
    closeFeaturesModal() {
        this._hideModal(this.elements.featuresModal);
    }
