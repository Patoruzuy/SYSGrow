...
(function () {
  ...
  function updateEntryFormFields() {
    ...
    switch (type) {
      ...
      case "nutrient":
        html = `
          <div class="mb-3">
            <label class="form-label">Nutrient Type</label>
            <select id="field-nutrient-type" class="form-select">
          ...
          <div class="mb-3">
            ...
            <input type="text" id="field-nutrient-name" class="form-control" placeholder="e.g. CalMag, Bloom A">
          </div>
          <div class="row g-3">
            <div class="col-sm-6">
              <label class="form-label">Amount</label>
              <input type="number" id="field-nutrient-amount" class="form-control" min="0" step="0.1" placeholder="5">
            </div>
            <div class="col-sm-6">
              <label class="form-label">Unit</label>
              <select id="field-nutrient-unit" class="form-select">
      ...
      case "note":
        html = `
          <div class="alert alert-light border mb-0" role="note">
            This will be saved as a general plant observation.
          </div>`;
        break;
      default:
        html = "";
  ...
  el("btn-save-entry")?.addEventListener("click", async () => {
    ...
    switch (type) {
      case "watering":
        requestPromise = api("/journal/watering", {
          method: "POST",
          body: JSON.stringify({
        ...
        break;
      case "observation":
        requestPromise = window.API.Plant.recordObservation({
          plant_id: plantId,
        ...
        break;
      case "pruning":
        requestPromise = api("/journal/pruning", {
          method: "POST",
          body: JSON.stringify({
            pruning_type: el("field-pruning-type")?.value || "other",
            notes,
          }),
        });
        break;
      case "transplant":
        requestPromise = api("/journal/transplant", {
          method: "POST",
          body: JSON.stringify({
        ...
        break;
      case "environmental_adjustment":
        requestPromise = api("/journal/environmental-adjustment", {
          method: "POST",
          body: JSON.stringify({
      ...
      case "note":
        ...
        break;
      case "nutrient": {
        const nutrientName = (el("field-nutrient-name")?.value || "").trim();
        const amount = parseFloat(el("field-nutrient-amount")?.value || "");
        if (!nutrientName || !Number.isFinite(amount)) {
          window.showNotification("Nutrient name and amount are required", "warning");
          return;
        }

        requestPromise = window.API.Plant.recordNutrient({
          plant_id: plantId,
          nutrient_type: el("field-nutrient-type")?.value || "custom",
          nutrient_name: nutrientName,
          amount,
          unit: el("field-nutrient-unit")?.value || "ml",
          notes,
        });
        break;
      }
    ...
    try {
      ...
      if (res?.success === false) {
        throw new Error(res.message || "Failed to save entry");
      }
      ...
      window.showNotification(res?.message || "Entry saved", "success");
    } catch (err) {
