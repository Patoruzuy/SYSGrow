...
(function () {
  ...
  class UnitsDataService {
    ...
    /**
     * Capture photo for unit camera
     */
    async capturePhoto(unitId) {
      try {
        const data = await this.api.Growth.capturePhoto(unitId);
        return { ok: true, data };
      } catch (error) {
    ...
    /**
     * Resolve camera feed URL for unit
     */
    getCameraFeedUrl(unitId) {
      return this.api.Growth.getCameraFeedUrl(unitId);
    }
