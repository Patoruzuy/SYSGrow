        ...
        for method, endpoint, description in test_cases:
            try:
                ...
                if status_code in working_codes:
                    ...
                    if (
                        "/growth-units/units" in endpoint
                        and "/plants" not in endpoint
                        and "/devices" not in endpoint
                        and "/camera" not in endpoint
                    ):
                        ...
                        "/api/devices" in endpoint or "/api/dashboard" in endpoint or "/api/v1/agriculture" in endpoint
                    ):
...
def _get_test_data_for_endpoint(endpoint):
    ...
    if "/units" in endpoint and endpoint.endswith("/units"):
        ...
        return {"plant_name": "Test Plant", "plant_type": "Lettuce", "current_stage": "seedling", "growth_stages": []}
    elif "/stage" in endpoint:
        return {"stage": "vegetative", "days_in_stage": 5}
    elif "/thresholds" in endpoint:
        return {"temperature_threshold": 24.0, "humidity_threshold": 60.0}
    else:
        return {}
