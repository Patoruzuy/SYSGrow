"""
Migration 050: Add timezone column to GrowthUnits.
...
MIGRATION_NAME = "add_unit_timezone"
...
def upgrade(db_path: str) -> bool:
    """Add timezone column to GrowthUnits table."""
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='GrowthUnits'"
        )
        if not cursor.fetchone():
            logger.error("GrowthUnits table not found; aborting migration")
            conn.close()
            return False

        if not _column_exists(cursor, "timezone"):
            cursor.execute("ALTER TABLE GrowthUnits ADD COLUMN timezone TEXT")
            logger.info("Added column timezone to GrowthUnits")
