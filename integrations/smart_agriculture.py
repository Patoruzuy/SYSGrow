...
class SmartAgricultureManager:
    ...
    def load_plants_data(self) -> Dict:
        ...
        try:
            with open(self.plants_json_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except FileNotFoundError:
            logger.error(f"Plants data file not found: {self.plants_json_path}")
            return {"plants_info": []}
    
    def get_plant_data(self, plant_id: int) -> Optional[Dict]:
        """Get plant data by ID"""
        return self.plant_lookup.get(plant_id)
    
    def get_watering_decisions(self, plant_id: int, current_moisture: float, 
                             last_watered: Optional[datetime] = None) -> Dict:
        """
    ...
    def check_environmental_alerts(self, plant_id: int, temperature: float, 
                                 humidity: float, soil_temp: Optional[float] = None) -> List[Dict]:
        """
    ...
    def calculate_yield_projection(self, plant_id: int, plants_count: int, 
    ...
    def get_harvest_recommendations(self, plant_id: int, days_since_planting: int) -> Dict:
        ...
        return {
            ...
            "harvest_window": {
                "min_days": harvest_min,
                "max_days": harvest_max
            },
            "status": status,
    ...
    def _get_watering_reasoning(self, current_moisture: float, moisture_range: Dict, 
                              trigger_point: float, urgency: str) -> str:
