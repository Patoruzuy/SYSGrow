...
(function () {
  ...
  class UnitsDataService {
    ...
    /**
     * Capture a photo for unit camera
     */
    async capturePhoto(unitId) {
      try {
        const response = await fetch(`/api/growth/units/${unitId}/camera/capture`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        });
        if (!response.ok) {
        ...
        const data = await response.json();
        return data.ok !== false ? { ok: true, data } : { ok: false, error: data.error || 'Failed to capture photo' };
      } catch (error) {
        console.error(`[UnitsDataService] capturePhoto(${unitId}) failed:`, error);
        return { ok: false, error: error.message || 'Failed to capture photo' };
      }
    }

    /**
     * Resolve camera feed URL for a unit
     */
